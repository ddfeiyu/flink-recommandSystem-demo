...
public class CosineSimilarity {
    ...
    public static double getSimilarity(String text1, String text2){
        ...
        // 第一步：进行分词
        List<Item> words1 = WordSegmentUtils.segment(text1);
        List<Item> words2 = WordSegmentUtils.segment(text2);
     ...
     * @return
     */
    public static double getSimilarity(List<Item> words1, List<Item> words2) {
     ...
     * @param words2
     */
    public static double getSimilarityImpl(List<Item> words1, List<Item> words2) {
        // 向每一个Item对象的属性都注入weight（权重）属性值
        taggingWeightByFrequency(words1, words2);

        // 第二步：计算词频
        // 通过上一步让每个Item对象都有权重值，那么在封装到map中（key是词，value是该词出现的次数（即权重））
        Map<String, Float> weightMap1 = getFastSearchMap(words1);
        Map<String, Float> weightMap2 = getFastSearchMap(words2);

        // 将所有词都装入set容器中
        Set<Item> allWords = new HashSet<>();
        ...
        // 第三步：写出词频向量，后进行计算
        allWords.parallelStream().forEach(new Consumer<Item>() {
            @Override
            public void accept(Item word) {
                // 看同一词在a、b两个集合出现的此次
        ...
        return BigDecimal.valueOf(ab.get())
                .divide(result, 9, BigDecimal.ROUND_HALF_UP).doubleValue();
    }
     ...
     * @param words2
     */
    protected static void taggingWeightByFrequency(List<Item> words1, List<Item> words2) {
     ...
     * @return 词频统计图
     */
    private static Map<String, AtomicInteger> getFrequency(List<Item> words) {
        ...
        for (Item i : words) {
             ...
             * }
             */
            // computeIfAbsent: 旧值为null且新值不为null时赋值
            ...
            // computeIfPresent:  如果新值为null，则remove，新值不为null，则覆盖旧的值
            /*
            freq.computeIfPresent(i.getName(), new BiFunction<String, AtomicInteger, AtomicInteger>(){
                @Override
                public AtomicInteger apply(String s, AtomicInteger newValue) {
                    return new AtomicInteger();
                }
            }).incrementAndGet();
            */
     ...
     * @return
     */
    protected static Map<String, Float> getFastSearchMap(List<Item> words) {
        if (CollUtil.isEmpty(words)) {
            return Maps.newConcurrentMap();
        }
        Map<String, Float> weightMap = new ConcurrentHashMap<>(words.size());

        words.parallelStream().forEach(new Consumer<Item>() {
            @Override
